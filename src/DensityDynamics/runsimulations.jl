...
"""
    A long trajectory is generated by parts (coincident with nsimulations)
"""
...
function geometrictrajectory(p::Parameters)

    beta = 1./p.T
    r0 = initialcondition(beta, p.Q)
    r0 = push!(r0, 0.0)
    tx = geometricintegration(r0,p)
    r0 = tx[end,:][2:end]
    println("Part 1 done")

    for i in 2:p.nsimulations
        traj = geometricintegration(r0,p)
        traj = traj[2:end,:]
        traj[:,1] += (i-1)*p.nsteps*p.dt
        tx = vcat(tx, traj)
        r0 = traj[end,:][2:end]
