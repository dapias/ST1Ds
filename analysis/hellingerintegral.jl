...
"""
This function returns a plain data text that contains the hellinger distance calculated in n (20 by default) different intervals of a trajectory together with its error. The normalization factor associated with the distribution in `q` is passed as an argument.
Example:
```
julia> hellingerintegral(filename, Potential("quartic", x->x^4./4.),
Thermostat("logistic", A, x -> exp(x/A)/(A*(1.+ exp(x/A)).^2)), normalizationfactor)
```
"""
function hellingerintegral(filename::String, potential::Potential,
  thermo::Thermostat, normalizationfactor::Float64, n = 20)
